...
type ApiHandler<Req> = (req: Req, res: NextApiResponse) => void | Promise<void>;
...
type RouteOptions<TBody, TQuery> = {
  ...
  | {
      protect: false;
      handler: ApiHandler<Request<TBody, TQuery>>;
    }
  | {
      protect?: true;
      handler: ApiHandler<AuthorizedRequest<TBody, TQuery>>;
    }
);
...
const validate = async <T extends z.ZodTypeAny>(o: any, schema: T): Promise<z.infer<T>> => {
  try {
    ...
    if (err instanceof z.ZodError) throw new HttpError(400, err.toString());
    throw err;
  }
};
...
const forwardErrors =
  (handler: ApiHandler<NextApiRequest>): ApiHandler<NextApiRequest> =>
  async (req, res) => {
    try {
...
export const route = <TBody = any, TQuery = any>(options: RouteOptions<TBody, TQuery>): ApiHandler<NextApiRequest> =>
  forwardErrors(async (_req, res) => {
