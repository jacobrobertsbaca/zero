...
export const PeriodListMutable = ({ form }: PeriodListMutableProps) => {
    const getValues = useCallback((period: Period) => {
        ...
        if (datesDays(period.dates) != period.days)
            values.push({ value: TruncateMode.Split, label: "Split" });
        return values;
    }, []);

    const onPeriodTruncateChanged = useCallback((index: number, event: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
