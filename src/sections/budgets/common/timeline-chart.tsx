...
const MID = "hsl(var(--muted-foreground))";
...
/** Vertical stroke color: solid when mono, otherwise green above zero / red below. */
function LineGradient({
  ...
  if (mono || min >= 0) return solid(POS);
  if (max <= 0) return solid(NEG);

  const z = Math.min(1, Math.max(0, (s(0) - y0) / (y1 - y0)));
  return (
    <defs>
      <linearGradient id={id} gradientUnits="userSpaceOnUse" x1={0} y1={y0} x2={0} y2={y1}>
...
export type TimelineChartProps = {
  ...
  /** When true, stroke/fill stay positive-green (Income/Spending metrics). */
  mono?: boolean;
  formatLabel?: (value: number) => string;
  resetKey?: string;
};
...
export function TimelineChart({
  ...
  const chartMargin = { top: 30, right: 0, left: 0, bottom: 16 };
  const shellClass = cn(
    ...
    "cursor-pointer [&_.recharts-surface]:cursor-pointer"
  );
